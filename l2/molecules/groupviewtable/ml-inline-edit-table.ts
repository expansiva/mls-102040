...
export class MlInlineEditTableMolecule extends MoleculeAuraElement {
  ...
  private getRowClasses(index: number, isSelected: boolean): string {
    return [
      ...
      this.focusedRowIndex === index ? 'ring-2 ring-inset ring-sky-500 dark:ring-sky-400' : '',
  ...
  private getCellClasses(): string {
    return ['px-4 py-3 text-sm', 'text-slate-900 dark:text-slate-100'].join(' ');
  }
  ...
  private renderCaption(): TemplateResult | typeof nothing {
    ...
    return html`
      <caption class="px-4 py-3 text-left text-lg font-semibold text-slate-900 dark:text-slate-100 bg-white dark:bg-slate-800">
        ${unsafeHTML(content)}
      </caption>
    `;
  }
  ...
  private renderEmpty(): TemplateResult {
    ...
    return html`
      <tbody role="rowgroup">
        <tr role="row">
          <td colspan="${colSpan}" class="px-4 py-12 text-center text-slate-500 dark:text-slate-400" role="cell">
            ${unsafeHTML(content)}
          </td>
        </tr>
      </tbody>
  ...
  private renderLoading(): TemplateResult {
    ...
    return html`
      <tbody role="rowgroup">
        ${[1, 2, 3].map(
          () => html`
            <tr role="row" class="border-b border-slate-200 dark:border-slate-700">
              ${this.selectable
                ? html`
  ...
  private renderPagination(): TemplateResult | typeof nothing {
    ...
    return html`
      <nav
        class="flex items-center justify-between px-4 py-3 bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700"
        role="navigation"
  ...
  private renderPaginationWithEllipsis(): TemplateResult {
    ...
    return html`
      ${pages.map((p) =>
        typeof p === 'string'
          ? html`<span class="px-2 text-slate-400 dark:text-slate-500">...</span>`
  ...
  private renderError(): TemplateResult | typeof nothing {
    ...
    return html`
      <div class="px-4 py-3 text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 border-t border-red-200 dark:border-red-800">
        ${this.error}
      </div>
    `;
  }
  ...
  render() {
    ...
    return html`
      <div
        class="overflow-hidden rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800"
