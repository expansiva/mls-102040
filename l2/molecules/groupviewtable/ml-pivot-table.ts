...
export class MlPivotTableMolecule extends MoleculeAuraElement {
  ...
  private getHeaderCellClasses(header: ParsedHeader): string {
    return [
      ...
      header.sortable && !this.disabled ? 'cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800 select-none' : '',
    ]
      .filter(Boolean)
      .join(' ');
  }
  ...
  private getBodyRowClasses(row: ParsedRow, isSelected: boolean): string {
    return [
      'transition-colors',
      row.isSubtotal || row.isTotal ? 'font-semibold bg-slate-100 dark:bg-slate-700' : '',
      isSelected && !row.isSubtotal && !row.isTotal ? 'bg-sky-50 dark:bg-sky-900/40' : '',
      !row.isSubtotal && !row.isTotal && !isSelected ? 'hover:bg-slate-50 dark:hover:bg-slate-700/50' : '',
  ...
  private getPaginationButtonClasses(isDisabled: boolean): string {
    return [
      ...
      isDisabled
        ...
        : 'text-slate-700 dark:text-slate-200 hover:bg-slate-200 dark:hover:bg-slate-700 cursor-pointer',
    ].join(' ');
  }
  ...
  private renderCaption(): TemplateResult {
    ...
    return html`
      <div class="px-4 py-3 font-semibold text-lg text-slate-900 dark:text-slate-100 border-b border-slate-200 dark:border-slate-700">
  ...
  private renderError(): TemplateResult {
    ...
    return html`
      <p class="mt-2 text-xs text-red-600 dark:text-red-400 px-4 pb-3">
