...
export class MlKanbanBoardMolecule extends MoleculeAuraElement {
  ...
  private getColumnClasses(isDropTarget: boolean): string {
    return [
      ...
      'rounded-lg',
      isDropTarget ? 'ring-2 ring-sky-500 dark:ring-sky-400' : '',
    ]
      .filter(Boolean)
      .join(' ');
  }
  ...
  private getCardClasses(row: ParsedRow, isDragging: boolean): string {
    ...
    return [
      ...
      !row.disabled && this.hoverable ? 'hover:shadow-md hover:border-slate-300 dark:hover:border-slate-600' : '',
  ...
  private getAddCardClasses(disabled: boolean): string {
    return [
      ...
      disabled ? 'opacity-50 cursor-not-allowed' : 'hover:border-sky-500 dark:hover:border-sky-400 hover:text-sky-600 dark:hover:text-sky-400',
    ]
      .filter(Boolean)
      .join(' ');
  }
  ...
  private renderLoading(): TemplateResult {
    ...
    return html`
      <div
        class="flex items-center justify-center min-h-96 bg-slate-50 dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700"
        aria-busy="true"
      >
        <div class="flex flex-col items-center gap-3 text-slate-500 dark:text-slate-400">
          <div class="w-8 h-8 border-4 border-slate-300 dark:border-slate-600 border-t-sky-500 dark:border-t-sky-400 rounded-full animate-spin"></div>
  ...
  private renderBoard(): TemplateResult {
    return html`
      <div class="flex gap-4 overflow-x-auto p-4 bg-slate-100 dark:bg-slate-900 rounded-lg min-h-96">
  ...
  private renderColumn(column: ParsedColumn, colIndex: number): TemplateResult {
    ...
    if (this.selectedIndices.includes(colIndex) || row?.selected) {
      ariaAttrs['aria-selected'] = 'true';
    }
    ...
    return html`
      <div
        ...
        ...=${this.spreadAttributes(ariaAttrs)}
      >
  ...
  private spreadAttributes(attrs: Record<string, string>): string {
    return '';
  }
    ...
    return html`
      <div class=${headerClasses}>
        <div class="flex items-center justify-between">
          ...
          ${column.content
            ? html`<div class="text-xs text-slate-500 dark:text-slate-400">${unsafeHTML(column.content)}</div>`
            : html``}
        </div>
      </div>
    `;
    ...
    if (!row) {
      return html`
        <div class="flex-1 p-3">
          <div class="text-slate-400 dark:text-slate-500 text-sm text-center py-4">
            ${this.msg.empty}
          </div>
        </div>
      `;
  ...
  private renderCard(row: ParsedRow, cell: ParsedCell, rowIndex: number, cellIndex: number): TemplateResult {
    ...
    return html`
      ...
      >
        <div class="text-slate-900 dark:text-slate-100 text-sm">
