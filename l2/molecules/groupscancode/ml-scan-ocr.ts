...
export class MlScanOcrMolecule extends MoleculeAuraElement {
  ...
  private renderResult(): TemplateResult {
    ...
    return html`
      <div
        class="w-full rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4"
        aria-live="polite"
      >
        ...
        ${hasCustom
          ...
          : html`<div class="text-sm text-slate-900 dark:text-slate-100">${this.value}</div>`}
      </div>
    `;
  }
  ...
  private renderCamera(): TemplateResult {
    const captureButtonClasses = [
      ...
      'border-slate-200 dark:border-slate-700',
      'hover:bg-slate-50 dark:hover:bg-slate-700',
      this.loading ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer',
    ]
      .filter(Boolean)
      .join(' ');

    return html`
      <div class="relative w-full overflow-hidden rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
        <video
        ...
        <div
          class="pointer-events-none absolute inset-6 rounded-lg border-2 border-dashed border-sky-500 dark:border-sky-400"
        ></div>

        ${this.isCapturing
          ? html`<div class="absolute inset-0 bg-sky-200/40 dark:bg-sky-500/30"></div>`
          : html``}
      </div>
  ...
  render() {
    ...
    return html`
      <div class=${rootClasses} @keydown=${this.handleRootKey}>
        ${this.renderLabel()}
