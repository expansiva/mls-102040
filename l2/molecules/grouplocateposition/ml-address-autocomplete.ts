...
export class AddressAutocompleteMolecule extends MoleculeAuraElement {
  ...
  private getSuggestionItemClasses(isSelected: boolean): string {
    return [
      ...
      !isSelected ? 'hover:bg-slate-50 dark:hover:bg-slate-700' : '',
    ]
      .filter(Boolean)
      .join(' ');
  }
  ...
  private renderLabel() {
    ...
    return html`<label id=${this.labelId} class="mb-1 block text-sm text-slate-600 dark:text-slate-400">
  ...
  private renderHelperOrError(isError: boolean) {
    ...
    if (isError) {
      return html`<p id=${this.errorId} class="mt-1 text-xs text-red-600 dark:text-red-400">
        ${unsafeHTML(this.error)}
      </p>`;
    }
    if (this.hasSlot('Helper')) {
      return html`<p id=${this.helperId} class="mt-1 text-xs text-slate-500 dark:text-slate-400">
  ...
  private renderEmptyState() {
    ...
    return html`<div class="px-3 py-2 text-sm text-slate-500 dark:text-slate-400">${unsafeHTML(emptyContent)}</div>`;
  }
  ...
  private renderSuggestionsPanel() {
    ...
    return html`
      <div class=${this.getSuggestionPanelClasses()} role="listbox">
        ${this.loading
          ? html`<div class="px-3 py-2 text-sm text-slate-500 dark:text-slate-400">${this.msg.loading}</div>`
          : items.length === 0
  ...
  private renderGeolocationButton() {
    ...
    const triggerContent = this.hasSlot('Trigger')
      ...
      : html`<span class="text-sm text-slate-600 dark:text-slate-300">${this.msg.useLocation}</span>`;
    return html`
      <button
        type="button"
        class="ml-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 text-sm text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700"
  ...
  private renderMapPreview() {
    ...
    return html`
      <div class="mt-3 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 p-3 text-xs text-slate-600 dark:text-slate-400">
        <div class="font-semibold text-slate-700 dark:text-slate-300">${coords.lat}, ${coords.lng}</div>
        <div class="mt-1 text-slate-500 dark:text-slate-400">Map preview</div>
      </div>
    `;
  }
  ...
  private renderViewMode() {
    ...
    return html`
      <div class="w-full">
        ${this.renderLabel()}
        <div class="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 text-sm text-slate-900 dark:text-slate-100">
          ${unsafeHTML(content)}
        </div>
