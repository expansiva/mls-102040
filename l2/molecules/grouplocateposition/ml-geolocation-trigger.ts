...
export class MlGeolocationTriggerMolecule extends MoleculeAuraElement {
  ...
  private renderHelperOrError(errorText: string, helperId: string): TemplateResult {
    ...
    if (errorText) {
      return html`<p id=${helperId} class="mt-1 text-xs text-red-600 dark:text-red-400">${unsafeHTML(errorText)}</p>`;
    }
  ...
  private renderTrigger(effectiveLoading: boolean): TemplateResult {
    ...
    const buttonClasses = [
      ...
      (this.disabled || this.readonly || effectiveLoading) ? 'opacity-50 cursor-not-allowed' : '',
    ].filter(Boolean).join(' ');
    return html`
      <button
        type="button"
    ...
    return html`
      <div class=${this.getPanelClasses()} role="listbox">
        ${effectiveLoading ? html`
          <div class="px-3 py-2 text-sm text-slate-500 dark:text-slate-400">${this.msg.loading}</div>
        ` : ''}
  ...
  private renderViewMode(valueLabel: string): TemplateResult {
    ...
    return html`
      <div class="rounded-lg border px-3 py-2 text-sm bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 border-slate-200 dark:border-slate-700">
        ${unsafeHTML(display)}
      </div>
    `;
  }
  ...
  private renderMapPreview(): TemplateResult {
    ...
    return html`
      <div class=${this.getMapClasses()}>
        ...
        <div class="mt-2 flex items-center justify-center rounded-md bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-3">
          <svg viewBox="0 0 64 64" class="h-16 w-16 text-sky-600 dark:text-sky-400" fill="currentColor" aria-hidden="true">
            ${svg`
